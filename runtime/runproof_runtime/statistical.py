...
def build_sampling_plan(
    *,
    ...
    suite_id: str = "rpf-incident-remediation-statistical-suite",
...
def validate_sampling_plan(plan: Mapping[str, Any]) -> list[str]:
    ...
    if source.get("runtime_version") != STATISTICAL_RUNTIME_VERSION or not source.get("source_sha256"):
...
def validate_statistical_evaluation(evaluation: Mapping[str, Any]) -> list[str]:
    ...
    for key in required:
        if key not in metadata:
            errors.append(f"MISSING_{key.upper()}")
    if metadata.get("evaluation_status") not in {"COMPLETE", "INCONCLUSIVE", "INVALID"}:
    ...
    for trial in trials:
        ...
        if index is None or index in trial_indexes:
            errors.append("TRIAL_INDEX")
        if trial.get("outcome") not in TRIAL_OUTCOMES:
            errors.append("TRIAL_OUTCOME")
        if trial.get("agent_quality_eligible") is not (trial.get("outcome") in AGENT_OUTCOMES):
    ...
    for family in summary.get("failure_families", []) if isinstance(summary.get("failure_families"), list) else []:
        if not isinstance(family, dict) or _int(family.get("denominator")) != expected_valid:
...
def build_statistical_comparison(
    ...
    *,
    ...
    if not errors:
        for key in ("compatibility_key", "scenario_ref"):
    ...
    base_quality = _obj(_obj(base.get("summary")).get("agent_quality"))
    candidate_quality = _obj(_obj(current.get("summary")).get("agent_quality"))
    base_evidence = _obj(_obj(base.get("summary")).get("evidence_quality"))
    candidate_evidence = _obj(_obj(current.get("summary")).get("evidence_quality"))
    ...
    adequate = bool(_obj(_obj(base.get("summary")).get("adequacy")).get("evidence_sufficient")) and bool(_obj(_obj(current.get("summary")).get("adequacy")).get("evidence_sufficient"))
    ...
    elif len(_obj(_obj(current.get("summary")).get("zero_tolerance")).get("events", [])) > len(_obj(_obj(base.get("summary")).get("zero_tolerance")).get("events", [])):
    ...
    base_family_list = _obj(base.get("summary")).get("failure_families")
    candidate_family_list = _obj(current.get("summary")).get("failure_families")
    ...
    aggregate = {
        ...
        "flaky_state": {"baseline": _text(_obj(_obj(base.get("summary")).get("flaky")), "state"), "candidate": _text(_obj(_obj(current.get("summary")).get("flaky")), "state")},
        "zero_tolerance_event_count": _metric_delta(_obj(_obj(base.get("summary")).get("zero_tolerance")).get("event_count"), _obj(_obj(current.get("summary")).get("zero_tolerance")).get("event_count")),
...
def build_statistical_policy(*, policy_id: str = "rpf-incident-statistical-policy", policy_version: str = "1.0.0") -> dict[str, Any]:
    metadata = {
        ...
        "compatible_sampling_plan": {"suite_id": "rpf-incident-remediation-statistical-suite", "suite_version": "1.0.0"},
        "rules": {
...
def evaluate_statistical_gate(
    ...
    *,
    ...
    quality = _obj(summary.get("agent_quality"))
    evidence = _obj(summary.get("evidence_quality"))
    adequacy = _obj(summary.get("adequacy"))
    zero = _obj(summary.get("zero_tolerance"))
    ...
    valid_count = int(evaluation_meta.get("valid_agent_trial_count", 0) or 0)
    minimum_valid = int(rules.get("minimum_valid_trial_count", 0) or 0)
    ...
    results = [
        ...
        _rule_result("minimum-valid-trials", "EVIDENCE", "PASS" if metric_ready else "FAIL", value=evaluation_meta.get("valid_agent_trial_count"), threshold=rules.get("minimum_valid_trial_count"), reason="Valid Agent trial denominator is sufficient." if metric_ready else "Valid Agent trial denominator is below the Sampling Plan minimum."),
    ...
    flaky_state = _text(_obj(summary.get("flaky")), "state")
    if flaky_state == "OBSERVED_FLAKY":
        results.append(_rule_result("flaky-observation", str(rules.get("observed_flaky_effect")), "FAIL", value=flaky_state, threshold="NO_FAILURE_OBSERVED", reason="PASS and ordinary FAIL were both observed; review is required and no live probability is claimed."))
    else:
        results.append(_rule_result("flaky-observation", str(rules.get("observed_flaky_effect")), "PASS", value=flaky_state, threshold="NO_FAILURE_OBSERVED", reason="No ordinary PASS/FAIL coexistence was observed."))
    blocking = [item for item in results if item.get("gate") == "HARD" and item.get("status") == "FAIL"]
    ...
    metadata = {
        ...
        "summary_facts": {"agent_quality": copy.deepcopy(quality), "evidence_quality": copy.deepcopy(evidence), "flaky": copy.deepcopy(_obj(summary.get("flaky"))), "zero_tolerance": copy.deepcopy(zero)},
