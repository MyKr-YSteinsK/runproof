...
- Lifecycle: `Prototype`
- Plan: `RPF-00 = Complete`；`RPF-01 = Complete`；`RPF-02 = Complete`；`RPF-03 = Complete`；`RPF-04 = Complete`；`RPF-05 = Complete`：已建立真实 Agent `FAIL`、Platform/Environment `ERROR`、独立 `rpf-failure-case-v1` 复现/验证闭环，并扩展只读 Desktop Web Evidence / Failure Investigation 控制面。
- Branch: `main`。
- Product / published version: 无。
- Last verified: 2026-09-11；RPF-01 的 14 项 synthetic/local 合同测试、RPF-02 的 7 项 host 生命周期测试与 1 项真实 Docker provider 集成测试、RPF-03/RPF-05 runtime 的 13 项 Python 合同/集成测试、Web 的 5 项 adapter tests、frontend typecheck/build 与 reviewed artifact verifier 均通过；正常和 response-lost 两条真实 DeepSeek + Docker v2 live Run 均为 `PASS`，RPF-05 的 Agent FAIL source/reproduction 与 controlled Environment ERROR 均通过真实 Docker runtime；浏览器在 1440/1280 desktop viewport 的入口、FAIL/ERROR/Failure Case 调查、来源/复现 event deep-link 与 build preview 均通过，8 份固定 JSON artifact 的源码身份/状态/secret 边界离线核验通过。
...
- 正式产品 runtime / Provider adapter 已有 RPF-03/RPF-05 的单 Run Prototype 实现，Desktop Web Control Plane 已有 RPF-05 的只读本地 reviewed corpus surface；Java Control Plane、API/backend service、DB、Queue、scheduler 与 durable worker 仍无。credential/私有 continuation 不持久化。
...
- RPF-05 Web 仍属于 read-only local/static adapter；尚未实现 API ingestion、Baseline/Candidate、Failure→Regression promotion、Evaluation suite 或 live Run/reproduce action。当前浏览器证据来自桌面 Chromium；`Real-device verification: not executed`，不据此宣称 iOS/mobile/PWA 体验。
...
无进行中的产品实现任务；RPF-05 已在真实 FAIL/ERROR corpus、Failure Case 复现验证与只读调查 surface 范围完成并交付，未把 Docker 写成永久产品架构决定，也未新增 canonical durable decision。

## Next likely boundary

进入下一 Reliability workflow boundary：在明确产品规则后调查 Failure Case → Regression、Baseline/Candidate 与 Evaluation suite 语义；保留 RPF-05 的 v2 evidence/v1 case adapter 与只读边界，在后续 Plan 明确数据来源前不引入 API/backend、Queue、scheduler 或 Durable framework。
