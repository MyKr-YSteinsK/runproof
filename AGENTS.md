...
- RPF-11 正式 Control Plane：`mvn -q test package -f control-plane/pom.xml`、`python control-plane/probe.py` 覆盖 Java/Spring + PostgreSQL canonical metadata、versioned schema history、immutable local `ArtifactStore`、completed-result manifest ingest、idempotency/conflict/concurrent race、transaction rollback、append-only/superseding Release Decision、service/read/evidence/decision/agent/CI authority、artifact fail-closed、service/DB restart 与 `pg_dump`/独立 `pg_restore`。probe 使用现有 Docker daemon 创建带 `rpf11` 前缀的临时资源，输出仅在忽略的 `.local/rpf-11/`；正式 Python client 为 `python -m runtime.runproof_runtime.control_plane_client`，只走 HTTP/JSON，不直连 DB。正式模块仍不包含 OAuth/OIDC/SSO、tenant/RBAC、Approval、CI provider workflow、Queue、scheduler、durable worker 或 release/deploy authorization。
- RPF-12 真实 GitHub Actions Release Gate：`.github/workflows/release-gate.yml` 由 `push(main)`、`pull_request(main)` 与 `workflow_dispatch` 触发；先执行 repository contract checks，再运行 `python ci/run_release_gate.py`，在 GitHub-hosted runner 内 fresh 启动 PostgreSQL + 正式 `control-plane/`，执行 fresh Baseline/Candidate Evaluation、独立 principal authority checks、canonical Decision read-back，并上传脱敏 `ci-results/rpf12-release-gate-result.json`、Summary mirror，同时写入并校验 GitHub Job Summary。workflow 只有 `contents: read`、`actions: write`，不执行 release/deploy；RPF-12 的真实 run、artifact、summary 与 remote delivery facts 仅以 CURRENT_STATE 为准。
