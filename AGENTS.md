...
- RPF-02 环境生命周期测试：`node --test spikes/rpf-02/probe.test.mjs`；固定证据离线校验：`node spikes/rpf-02/verify-evidence.mjs`；参考实验：`node spikes/rpf-02/probe.mjs --run`。这些命令不调用 Provider；实验仅使用独立临时根目录与 Node worker，不把 Docker 不可用时的结果冒充正式隔离证明。
...
- RPF-02 临时输出仅在忽略的 `.local/rpf-02/`；`spikes/rpf-02/reviewed-evidence.json` 是经过脱敏审查的 host-reference 固定样本，允许提交，不能自动以新实验覆盖。修改 probe 后需重新生成并审查样本的 `source_sha256`，同时保留 `Partial` 与 Docker daemon 不可用边界。
