...
- RPF-12 真实 GitHub Actions Release Gate：`.github/workflows/release-gate.yml` 由 `push(main)`、`pull_request(main)` 与 `workflow_dispatch` 触发；先执行 repository contract checks，再运行 `python ci/run_release_gate.py`，在 GitHub-hosted runner 内 fresh 启动 PostgreSQL + 正式 `control-plane/`，执行 fresh Baseline/Candidate Evaluation、独立 principal authority checks、canonical Decision read-back，并上传 `ci-results/rpf12-release-gate-result.json` 与 Job Summary。workflow 只有 `contents: read`、`actions: write`，不执行 release/deploy；RPF-12 的真实 run、artifact、summary 与 remote delivery facts 仅以 CURRENT_STATE 为准。
