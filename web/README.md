# RPF-06 Evidence / Regression Control Plane

这是第一个正式 Desktop Web Control Plane，默认消费仓库内真实、脱敏、源码绑定的 `rpf-run-evidence-v2` Run Evidence、独立 `rpf-failure-case-v1` Failure Case、`rpf-regression-v1` Regression、focused result 与 Historical Regression collection reviewed artifacts。数据通过 `web/src/data/artifacts.ts` 的 adapter 进入 UI；presentation components 不直接依赖 artifact 文件路径的字段细节。
